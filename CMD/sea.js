...
!(function (a, b) {
  ...
  function f(a) {
    for (a = a.replace(E, "/"), a = a.replace(G, "$1/"); a.match(F); )
      a = a.replace(F, "/");
    return a;
  }
  function g(a) {
  ...
  function r() {
    ...
    for (
      var a = _.getElementsByTagName("script"), b = a.length - 1;
      b >= 0;
      b--
    ) {
      var c = a[b];
  ...
  function s(a) {
    ...
    function h() {
      ...
      (p = { if: 1, for: 1, while: 1, with: 1 }[c]),
        ...
        o
          ? ((c = /^require\s*\(\s*['"]/.exec(b)[0]), (k += c.length - 2))
          : (k += /^[\w$]+(?:\s*\.\s*[\w$]+)*/.exec(b)[0].length - 1);
    }
    function i() {
    ...
    function j() {
      ...
      (c =
        "." == l
          ? /^\.\d+(?:E[+-]?\d*)?\s*/i.exec(b)[0]
          : /^0x[\da-f]*/i.test(b)
          ? /^0x[\da-f]*\s*/i.exec(b)[0]
          : /^\d+\.?\d*(?:E[+-]?\d*)?\s*/i.exec(b)[0]),
        (k += c.length - 1),
        (n = 0);
    }
    if (-1 == a.indexOf("require")) return [];
    for (
      var k = 0, l, m = a.length, n = 1, o = 0, p = 0, q = [], r = [];
      m > k;
